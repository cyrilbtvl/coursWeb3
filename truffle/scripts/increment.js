...
module.exports = async function (callback) {
  ...
  const currentGreeter = (await deployed.greet());
  console.log(`Current SimpleStorage value: ${currentGreeter}`);

  const { txGreeter } = await deployed.setGreeter(currentValue + 1);
  console.log(`Confirmed transaction ${txGreeter}`);

  const updatedGreeter = (await deployed.greet());
  console.log(`Updated SimpleStorage value: ${updatedGreeter}`);
